...
% Code identifies the maximum likelihood estimated fits of the Exponential distribution family to the interspike intervals

clear;
close all;

foldername='Exponential'; % Distribution type
...
% Pre-allocate structure fieldnames
...
exp_pdf=@(x,lambda) lambda*exp(-lambda*x); % Interspike Interval distribution
num_pars=1; % Number of parameters
...
for ii=1:n_exps
    ...
    while jj<nRuns+1
        ...
        % Attempt MLE fit
        try
        [opt_pars,pci]=mle(isi,'pdf',exp_pdf,'start',pars0,'LowerBound',lb,'UpperBound',ub,'Options',opts);
        
        lambda=opt_pars(1);
        negL=-sum(log(exp_pdf(isi,lambda)));
    ...
    lambda_opt=opt_pars(1); % Rename optimal parameter
    logL=sum(log(exp_pdf(isi,lambda_opt))); % Re-calculate log-likelihood

    AIC=2*num_pars-2*logL; % Akaike Information Criterion
    BIC=num_pars*log(numobs)-2*logL; % Bayesian Information Criterion

    mean_pdf=1/lambda_opt; % Distribution Mean
    variance_pdf=1/(lambda_opt^2); % Distribution Variance
...
save_name=['.\' foldername  '\Exponential_' totalname '.mat'];
