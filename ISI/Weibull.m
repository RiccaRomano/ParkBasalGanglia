...
foldername='weibull'; % Distribution type
...
weibull_pdf=@(x,k,lambda) k/lambda*((x/lambda).^(k-1)).*exp(-(x/lambda).^k);
...
for ii=1:n_exps
    ...
    while jj<nRuns+1
        ...
        pars0=(ub-lb).*rand(1,num_pars); % Initial parameter values
        
        try
        [opt_pars,pci]=mle(isi,'pdf',weibull_pdf,'start',pars0,'LowerBound',lb,'UpperBound',ub,'Options',opts);
        
        k=opt_pars(1);
        lambda=opt_pars(2);
        negL=-sum(log(weibull_pdf(isi,k,lambda)));
    ...
    logL=sum(log(weibull_pdf(isi,k_opt,lambda_opt)));
    
    % Calculate Akaike and Bayesian Information Criteria
    AIC=2*num_pars-2*logL;
    BIC=num_pars*log(numobs)-2*logL;

    mean_pdf=lambda*gamma(1+1/k); % Distribution mean
    variance_pdf=lambda^2*(gamma(1+2/k)-(gamma(1+1/k))^2); % Distribution variance
