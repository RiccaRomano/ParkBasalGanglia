...
if explosive(kern)
    ...
    sddb = nan; 
    kernc = nan; 
    disp('Error: estimated model is explosive.');
    return;
end
...
elseif length(kern)==b/2
    ...
    % Call fmincon
    options = optimoptions('fmincon', 'Display', 'iter', 'Algorithm', 'sqp',  'ConstraintTolerance', 0.01, 'MaxFunctionEvaluations', 10000, 'StepTolerance', 1e-15);
...
for k = (m+1):order
    for t = 1:k
        log_nCk = gammaln(k + 1) - gammaln(t + 1) - gammaln(k - t + 1);
        nCk_value = exp(log_nCk);
        x(t,k-m) = nCk_value*((-1)^(t+1));
    end
...
% kernc1 = x(:, 1) + X*v_qr;
% 
% % QR decomposition with column pivoting (handles rank-deficient matrices better)
% [Q, R, P] = qr(X, 'econ');
% c = Q' * bhat;
% tol = max(size(X)) * eps(class(X));
% rank_R = sum(abs(diag(R)) > tol);
% x_p = zeros(size(X, 2), 1);
% if rank_R > 0
%     x_p(1:rank_R) = R(1:rank_R, 1:rank_R) \ c(1:rank_R);
% end
% x_temp = P * x_p;
% kernc2=x(:,1)+X*x_temp;
% 
% % SVD works better for rank deficient matrices
% [U,S,V] = svd(X);
% S_hat=[diag(1./diag(S)) zeros(order-1,1)];
% X_dag=V*S_hat*U';
% kernc3=x(:,1)+X*X_dag*(bhat);
...
% AR model fitting - Maximum Likelihood
...
% AR model fitting - Yule-Walker method
...
dim = length(kern)-b/2;
figure; 
pl=tiledlayout(dim,1,'TileSpacing','compact','Padding','compact');
ylabel(pl,'KL rate (bits/s)','FontSize',14);
